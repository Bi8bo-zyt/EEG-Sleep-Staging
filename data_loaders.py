...
class LoadDataset_from_numpy(Dataset):
    ...
    def __init__(self, np_dataset):
        ...
        if not os.path.exists(np_dataset[0]):
            raise FileNotFoundError(f"File not found: {np_dataset[0]}")

        # 检查文件是否是有效的 zip 文件
        if not zipfile.is_zipfile(np_dataset[0]):
            raise zipfile.BadZipFile(f"File is not a zip file: {np_dataset[0]}")
